package com.leetcode.fred;

/**
 * Created by fred_chen on 7/2/15.
 */
public class FindMinInRotatedSortedArray
{
    public static int findMin(int[] nums)
    {
        int min = Integer.MAX_VALUE;
        int len = nums.length;
        int left = 0;
        int right = nums.length - 1;
        int pos = (left + right )/2;

//        System.out.println("Array Len: "+len+"");


        while(true)
        {
//            System.out.println("Left:[" + left +"]["+ nums[left]
//                            +"] Pos :[" + pos +"]["+ nums[pos]
//                            +"] Right:[" + right +"][" + nums[right] + "]");
            TREND pos_trend = getTrend(pos, nums);
            if (pos_trend == TREND.BOTTOM)
            {
                min = nums[pos];
                break;
            }
            else if (pos_trend == TREND.UP)
            {
                TREND scope_trend = getTrend(nums[pos], nums[left], nums[right]);
                if ( scope_trend == TREND.BOTTOM)
                {
                    right = pos;
                    pos = (pos + left)/2;
                }
                else if ( scope_trend == TREND.TOP)
                {
                    left = pos;
                    pos = (pos + right)/2;
                }
                else if ( scope_trend == TREND.UP)
                {
                    min = nums[left];
                    break;
                }

            }
            else if (pos_trend == TREND.DOWN)
            {
                TREND scope_trend = getTrend(nums[pos], nums[left], nums[right]);
                if ( scope_trend == TREND.TOP)
                {
                    right = pos;
                    pos = (pos + left)/2;
                }
                else if ( scope_trend == TREND.BOTTOM)
                {
                    left = pos;
                    pos = (pos + right)/2;
                }
                else if ( scope_trend == TREND.DOWN)
                {
                    min = nums[right];
                    break;
                }
            }
            else
            {
                int l = getLeftPos(pos, len);
                int r = getRightPos(pos, len);
                if (nums[l] > nums[r])
                {
                    min = nums[r];
                }
                else
                {
                    min = nums[l];
                }
                break;
            }
        }

        return min;

    }

    private static int getLeftPos(int pos, int length)
    {
        int left = pos - 1;
        if (left < 0)
        {
            left = length - 1;
        }

        return left;
    }

    private static int getRightPos(int pos, int length)
    {
        int right = pos + 1;
        if (right >= length)
        {
            right = 0;
        }

        return right;
    }

    enum TREND{BOTTOM, TOP, UP, DOWN};

    private static TREND getTrend(int pos, int left, int right)
    {
        if (pos < left && pos < right)
        {
            return TREND.BOTTOM;
        }
        else if (pos > left && pos > right)
        {
            return TREND.TOP;
        }
        else if (pos > left && pos < right)
        {
            return TREND.UP;
        }
        else
        {
            return TREND.DOWN;
        }
    }

    private static TREND getTrend(int pos, int[] nums)
    {
        int left = getLeftPos(pos, nums.length);
        int right = getRightPos(pos, nums.length);
        return getTrend(nums[pos], nums[left], nums[right]);
    }


    public static void main(String[] args)
    {
        int nums[] = {4,5,6,7,8,9, 10, 11, -2, -1, 0,1,2,3};

        System.out.println("Min in Nums: [" + findMin(nums) + "]");
    }
}
